...
# Pegue seu token da Hugging Face como variável de ambiente
...
def classify_sentiment(text):
    ...
    data = resp.json()
    # data[0]['label'] vem como "5 stars", "1 star", etc.
...
def backfill():
    ...
    all_reviews = []
    while True:
        result, token = reviews(APP_ID, lang='pt_BR', sort=Sort.NEWEST,
...
def get_reviews():
    ...
    APP_ID = 'co.stone.banking.mobile.flagship'
    result, _ = reviews(APP_ID, lang='pt_BR', sort=Sort.NEWEST, count=20)
    output = []
    for r in result:
